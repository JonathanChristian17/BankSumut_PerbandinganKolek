...
from kolek.forms import HoneypotLoginForm

urlpatterns = [
    path('login/', auth_views.LoginView.as_view(
        template_name='kolek/login.html',
        authentication_form=HoneypotLoginForm
    ), name='login'),
    path('logout/', auth_views.LogoutView.as_view(), name='logout'),
    path('captcha/', include('captcha.urls')),    # Jalur server pengirim gambar teks Captcha
